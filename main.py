...
async def ask_query(request: Request):
    ...
    return JSONResponse({"answer": query_data(data.get("question", ""))})
...
async def upload_file(request: Request, tipo: str, file: UploadFile = File(...)):
    ...
    try:
        if tipo == "sites":
            ...
            msg = "Base de SITES atualizada!"
        else:
            process_excel_escala(file_path)
            msg = "ESCALA mensal atualizada!"
        res = templates.TemplateResponse("admin.html", {"request": request, "msg": msg})
    except Exception as e:
        res = templates.TemplateResponse("admin.html", {"request": request, "error": str(e)})
